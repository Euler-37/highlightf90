...
with open(name, 'r') as f:
    input_data=f.read()
functions=re.findall(r'(?i)(?:function|subroutine)\x20+(\w*)\x20*\(',input_data, re.DOTALL)
functions=list(map(lambda x:x.lower(),functions))
functions=list(dict.fromkeys(functions))
interface=re.findall(r'(?i)(?:interface)\x20+(\w*)',input_data, re.DOTALL)
interface=list(map(lambda x:x.lower(),interface))
interface=list(dict.fromkeys(interface))
generic  =re.findall(r'(?i)(?:generic|procedure)\x20*::\x20*(\w*)',input_data, re.DOTALL)
